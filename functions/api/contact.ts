...
export const onRequestPost: PagesFunction = async ({ request }) => {
  try {
    ...
    if (typeof apiKey !== "string" || !apiKey.trim()) {
      return withCors(jsonResponse({ ok: false, error: "missing_resend_api_key" }, { status: 500 }), request);
    }
    ...
    if (!response.ok) {
      ...
      return withCors(jsonResponse({ ok: false, error: "resend_error", details }, { status: 502 }), request);
    }
